...
type BuildChatInputProps = {
  ...
  placeholder?: string
  /**
   * `inline` (default): regular flex child — sits at the natural bottom
   * of its container, no overlay, no gap below. Use inside flex columns
   * where the sibling above is the scrollable chat thread.
   * `fixed`: pinned to the viewport bottom — used by full-page surfaces
   * like the Custom Tools catalog where the input must float above a
   * floating tab bar at the same z-layer.
   */
...
export function BuildChatInput({
  ...
  const capsule = (
    <div
      className="flex w-full max-w-[640px] items-end gap-2 rounded-3xl px-3 py-2.5"
      style={{
  ...
  if (position === 'inline') {
    // Sits as the last flex child in a flex column. The chat scroll
    // sibling above takes flex-1 and shrinks to make room. No overlay,
    // no absolute positioning — the input is part of the chat panel.
    return (
      <div
  ...
  // Fixed: floats over content (catalog page, above the floating tab bar).
  return (
    <div
    ...
    >
      <div className="pointer-events-auto w-full max-w-[640px]">{capsule}</div>
    </div>
  )
}
